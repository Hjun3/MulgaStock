...
function StockLineChart({ height = 130 }: { height?: number }) {
  ...
  if (isLoading) return <div className="animate-pulse rounded bg-slate-800" style={{ height }} />;
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <LineChart data={chartData} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
        <XAxis dataKey="d" tick={{ fontSize: 10, fill: '#6b7fa8' }} axisLine={false} tickLine={false} interval={interval} />
...
function HeroChartCard() {
  ...
  return (
    <Card>
      <p className="mb-1 text-xs text-slate-400">전체 물가지수 (3개월)</p>
      ...
      <div className="flex w-56 flex-shrink-0 flex-col gap-3">
        {activeSectors.map(s => (
          <Card key={s.category} className="flex-1">
            <p className="mb-1 text-xs text-slate-400">{s.displayName}</p>
            ...
            <p className="mt-1 text-xs text-slate-400">상위: {s.topGainer ? s.topGainer.name : ''}</p>
          </Card>
        ))}
      </div>
    </div>
...
function MoverRow({ stock }: { stock: StockSummary }) {
  return (
    <Link to={`/stocks/${stock.id}`} className="flex items-center justify-between py-1.5 text-sm hover:opacity-80">
      <span className="text-slate-100">{stock.name}</span>
...
function CategoryMovers({ category, label }: { category: string; label: string }) {
  ...
  return (
    <Card>
      <h3 className="mb-3 text-sm font-bold text-slate-100">{label}</h3>
      <p className="mb-1 text-xs font-semibold text-red-400">▲ 급등 TOP 3</p>
      {isLoading ? <Skeleton className="h-20" /> : topData.map(s => <MoverRow key={s.id} stock={s} />)}
      <div className="my-2 border-t border-slate-800" />
      <p className="mb-1 text-xs font-semibold text-blue-400">▼ 급락 TOP 3</p>
...
function Top10Table() {
  ...
  return (
    <Card>
      <h3 className="mb-4 text-sm font-bold text-slate-100">전체 등락률 TOP 10</h3>
      {isLoading ? (
        <div className="space-y-2">{[0,1,2,3,4].map(i => <Skeleton key={i} className="h-10" />)}</div>
      ) : (
        <>
          <div className="mb-2 flex gap-2 border-b border-slate-800 pb-2 text-xs font-semibold text-slate-400">
          ...
          {data && data.content.map((stock, i) => (
            <Link key={stock.id} to={`/stocks/${stock.id}`}
              className="flex items-center gap-2 border-b border-slate-800 py-2 text-sm transition-colors hover:bg-slate-800">
              <span className="w-7 text-slate-400">{i + 1}</span>
              <span className="flex-1 text-slate-100">{stock.name}</span>
              <span className="w-16 text-xs text-slate-400">{CAT_LABEL[stock.category as keyof CatLabel] || stock.category}</span>
              <span className="w-24 text-right text-slate-100">{formatPrice(stock.currentPrice)}</span>
...
export function LandingPage() {
  const FEATURES = [
    { icon: <TrendingUp className="h-6 w-6 text-slate-300" />, title: '실시간 물가 추적', desc: 'KAMIS · Opinet · ECOS API로 28종 품목 가격을 주기적으로 업데이트합니다.' },
    { icon: <Bot className="h-6 w-6 text-slate-300" />, title: 'AI 시황 분석', desc: '오늘의 물가 흐름을 AI가 주식 시황처럼 자동으로 요약해 제공합니다.' },
    { icon: <Star className="h-6 w-6 text-slate-300" />, title: '포트폴리오 관리', desc: '관심 품목을 즐겨찾기하고 나만의 생활물가 포트폴리오를 구성하세요.' },
  ];
    ...
    <>
      ...
      <section className="border-b border-slate-800">
        <div className="mx-auto flex max-w-7xl items-start gap-14 px-6 py-16 lg:px-10">
          <div className="flex-1 pt-2">
            <p className="mb-4 text-xs uppercase tracking-widest text-slate-400">소비자 물가 실시간 추적 서비스</p>
            <h1 className="mb-4 text-4xl font-extrabold leading-tight tracking-tight text-slate-100">
      ...
      <section className="border-b border-slate-800 py-12">
        <div className="mx-auto flex max-w-7xl gap-4 px-6 lg:px-10">
          {FEATURES.map(f => (
            <div key={f.title} className="flex-1 rounded-xl border border-slate-800 p-8 transition-colors hover:bg-slate-900">
              <div className="mb-3">{f.icon}</div>
              <p className="mb-2 text-sm font-bold text-slate-100">{f.title}</p>
      ...
      <div className="mx-auto max-w-7xl space-y-6 px-6 py-6 lg:px-10">
        <MarketOverview />
        <div>
          <h2 className="mb-3 text-base font-bold text-slate-100">카테고리별 급등·급락</h2>
