...
import type { MarketSummary, StockSummary, StockDetail, PriceHistory, Page, TopMover } from './types';

// .env 의 VITE_API_BASE_URL 값을 사용한다. (예: http://localhost:8080/api/v1)
const BASE_URL = import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080/api/v1';

// 공통 GET 함수
// 백엔드 응답이 { success, data, message, timestamp } 형태라서 data만 꺼낸다.
...
export function getTopMovers(type: 'gainers' | 'losers', limit = 3) {
  return get<TopMover[]>('/stocks/top-movers?type=' + type + '&limit=' + limit);
}
...
// 특정 카테고리(FOOD/ENERGY/MACRO ...)의 종목들을 등락률 높은 순으로
